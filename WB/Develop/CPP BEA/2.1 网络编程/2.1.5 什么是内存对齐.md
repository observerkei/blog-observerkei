...
7. 如果指定了`__attribute__((aligned(m)))` 用于修饰结构体，则表示结构体的大小$S_{struct}$是m的整数倍，也就是 $S_{sttuct} = k * m$ 其中k是正整数，m必须是2的正幂数。如 `typedef struct { int a; } __attribute__((aligned(16))) s_t;` 情况下，`s_t`的结构体大小 $S_{s\_t}=16k$ 其中k是正整数，表示结构体 `s_t` 的大小为16的倍数。
...
a是char，占用1字节，也就是说$S_a=S_{char}=1$,a的起始地址$P_a = k * min(n, S_a) = k * min(4, 1) = k$ k是正整数。a是第一个成员，现在默认首地址成员已经地址对齐，为了分析方便，让首地址成员的地址为0，取 $P_a = 0$ 内存分布示意图如下：  
```bash
a1 char
```
