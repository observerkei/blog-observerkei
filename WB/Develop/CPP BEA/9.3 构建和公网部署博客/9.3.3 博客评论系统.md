...
> 为什么要用 `Waline` ?   ~~因为免费~~ ， ~~因为搭建起来方便~~， 因为目前只找到这个最合适
...
可以让AI帮忙写个脚本在发布前手动执行以下就能自动插入代码（注意要检测是否已经插入过，不要重复插入，可以过滤是否存在`https://unpkg.com/@Waline/client@v3/dist/Waline.js` ，存在就不重复插入，并且要事先做好数据备份，防止把文件写坏。）
...
特别的，如果你想通过  `afterDOMLoaded` /`beforeDOMLoaded`  同时执行多个脚本，在其他脚本是 `export default` 情况下， 可以这样使用，然后在另一个地方直接引入这个文件赋值给  `afterDOMLoaded` /`beforeDOMLoaded`   属性（[[#`Quartz` 的组件集成方式]]）：
...
- 也不能直接使用 `Vue`，不支持直接写 `Vue`
- 需要通过 `Javascript` 字符串代码导入方式来写专用组件才能够被调用😂。直接写函数不行，直接 export 也不行，`Quartz` 框架组件代码执行方式限制了使用方法。
...
> 不是很想写前端代码，从 `Quartz` 的复杂插件传递方式来看他限制住了客户端编写 JSX 代码的的灵活性（目前必须通过`字符串`引入代码），这是后续可以优化的地方（就像上面提供的放在一个脚本里面的方法其实就可以在内部进行封装）， 以及 DOM 和他配套的 `Javascript` 代码分开存放就会导致项目文件变得很多，不易阅读（实际上你要改相关代码也得两个部分一起看才能下手），可能是出于性能考虑所以没有用 `React` 吧（~~随着设备性能的提高好像也不会差多少~~），
